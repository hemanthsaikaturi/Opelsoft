...
async function getHomeData() {
  try {
    ...
    return {
      jobs: [
        { id: 1, title: 'Senior AI/ML Engineer', job_type: 'Full-time', salary_package: '90000-140000', city: 'London', country: 'UK', company_name: 'DeepMind' },
        { id: 2, title: 'Robotics Engineer', job_type: 'Full-time', salary_package: '75000-110000', city: 'Cambridge', country: 'UK', company_name: 'Wayve' },
        { id: 3, title: 'Computer Vision Engineer', job_type: 'Contract', salary_package: '80000-120000', city: 'Remote', country: 'UK', company_name: 'Tractable' },
      ],
      industries: [{ industry: 'Artificial Intelligence', count: 24 }, { industry: 'Robotics', count: 18 }, { industry: 'Machine Learning', count: 14 }, { industry: 'Automation', count: 9 }],
...
const VALUES = [
  { tint: '#4F46E5', title: 'Quality Opportunities', body: 'Live roles from real, verified employers — startups to enterprises, across every major industry.', icon: <><rect x="2" y="7" width="20" height="14" rx="2" /><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16" /></> },
  { tint: '#7C3AED', title: 'Apply in Seconds', body: 'Build your profile once, add your CV, and apply to any role in a few clicks — no repetitive forms.', icon: <path d="M13 2 3 14h9l-1 8 10-12h-9l1-8z" /> },
  { tint: '#0EA5E9', title: 'Stay in Control', body: 'Track every application — submitted, shortlisted, hired — in one clean, real-time dashboard.', icon: <path d="M22 12h-4l-3 9L9 3l-3 9H2" /> },
];

const STEPS = [
  { tint: '#4F46E5', n: '01', title: 'Create your profile', body: 'Add your skills, experience, and CV — or upload a CV to fill it in for you.', icon: <><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></> },
  { tint: '#7C3AED', n: '02', title: 'Search & apply', body: 'Filter thousands of live roles and apply in clicks with your saved profile.', icon: <><circle cx="11" cy="11" r="8" /><path d="m21 21-4.3-4.3" /></> },
  { tint: '#10B981', n: '03', title: 'Track & get hired', body: 'Follow every application from submitted to offer, all in one place.', icon: <path d="M20 6 9 17l-5-5" /> },
];

const SERVICES = [
  { tint: '#4F46E5', name: 'Job Board', body: 'Thousands of live roles to search, filter, and apply to in minutes.', href: '/jobs', icon: <><rect x="2" y="7" width="20" height="14" rx="2" /><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16" /></> },
  { tint: '#7C3AED', name: 'Talent & Staffing', body: 'Contract, direct-hire, and executive search for AI, robotics, and deep-tech teams.', href: '/talent-staffing', icon: <><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75" /></> },
  { tint: '#0EA5E9', name: 'Candidate Profiles', body: 'A complete profile and CV you build once and reuse everywhere.', href: '/register', icon: <><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></> },
  { tint: '#10B981', name: 'Application Tracking', body: 'A single dashboard to follow every application from submitted to offer.', href: '/dashboard/candidate', icon: <path d="M22 12h-4l-3 9L9 3l-3 9H2" /> },
  { tint: '#F59E0B', name: 'Executive Search', body: 'Confidential senior-leadership and C-suite placement, matched to fit.', href: '/talent-staffing#solutions', icon: <><circle cx="12" cy="8" r="6" /><path d="M15.5 13.5 17 22l-5-3-5 3 1.5-8.5" /></> },
  { tint: '#EF4444', name: 'Career Resources', body: 'Guidance and a growing library to help you take the next step.', href: '/about-us', icon: <><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20" /><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z" /></> },
];
...
export default async function Home() {
  ...
  return (
    <div className="home-page" style={{ background: 'var(--bg-color)', color: 'var(--text-primary)' }}>
      ...
      {/* ── HERO ─────────────────────────────────────────── */}
      <section className="op-mesh" style={{ borderBottom: '1px solid var(--border-color)', padding: '80px 0 72px', overflow: 'hidden' }}>
      ...
      <section style={{ background: '#0B0B0F', color: '#fff', borderBottom: '1px solid var(--border-color)' }}>
        <div className="container" style={{ padding: '52px 0' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '24px', textAlign: 'center' }}>
            {[
              { v: stats.jobsCount, s: '+', label: 'Open roles' },
              { v: stats.companiesCount, s: '+', label: 'Hiring companies' },
              { v: stats.usersCount, s: '+', label: 'Candidates' },
              { v: industries.length || 6, s: '+', label: 'Industries' },
            ].map((m, i) => (
