...
export async function POST(request) {
  try {
    ...
    {
      "job_title": "string (main target role, e.g. Senior Software Engineer)", "skills": [{ "name": "string (e.g. React)", "percentage": 85 }], "education": [{ "title": "string", "institute": "string", "from": "string", "to": "string", "description": "string" }], "experience": [{ "title": "string", "company": "string", "from": "string", "to": "string", "description": "string" }]
    }
    ...
    const parsed = await callLLMForJson(parsePrompt, {
      system: 'You are a precise resume parser. Return ONLY valid JSON.', maxTokens: 2000, temperature: 0.1
    });
    ...
    let skillsArray = [
      { name: 'JavaScript', percentage: 90 }, { name: 'React', percentage: 85 }, { name: 'Node.js', percentage: 80 }, { name: 'SQL', percentage: 75 }
    ];

    if (fileName.includes('design') || fileName.includes('ui') || fileName.includes('ux')) {
      parsedTitle = 'Product Designer';
      skillsArray = [
        { name: 'Figma', percentage: 95 }, { name: 'UI/UX Design', percentage: 90 }, { name: 'HTML & CSS', percentage: 80 }, { name: 'Wireframing', percentage: 85 }
      ];
    } else if (fileName.includes('product') || fileName.includes('manager')) {
      parsedTitle = 'Product Manager';
      skillsArray = [
        { name: 'Product Roadmap', percentage: 95 }, { name: 'Agile/Scrum', percentage: 90 }, { name: 'Jira', percentage: 85 }, { name: 'SQL & Analytics', percentage: 70 }
      ];
    }

    const mockProfile = {
      job_title: parsedTitle, skills: skillsArray, education: [
        { 
          title: 'Bachelor of Science in Computer Science', institute: 'Adamas University', from: '2019', to: '2023', description: 'Graduated with first-class honors. Focus on software engineering methodologies.' 
        }
      ], experience: [
        { 
          title: parsedTitle + ' Intern', company: 'Opelsoft', from: '2022', to: '2023', description: 'Assisted in building recruitment systems and automation playbooks.' 
        }
      ]
    };

    return NextResponse.json({ 
      success: true, profile: mockProfile, fallback: true, message: 'Parsed via developer fallback (add GROQ_API_KEY in .env.local to enable real LLM parsing).'
    });

  } catch (error) {
    console.error('Resume upload endpoint error:', error);
    return NextResponse.json(
      { success: false, message: 'Server error during resume parsing.' }, { status: 500 }
    );
  }
}
